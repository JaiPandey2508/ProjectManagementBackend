...
const getProjects = asyncHandler(async (req, res) => {
  const projects = await ProjectMember.aggregate([
    {//first aggregation pipeline
      $match: {//matching the data, like the WHERE clause
...
const updateProject = asyncHandler(async (req, res) => {
  ...
  if(!project){//if we don't get a project, we throw an error
    throw new ApiError(404, "Project not found")
  }
  return res//if project found, send this res
    .status(200)
    .json(
      new ApiResponse(
        200,
...
const deleteProject = asyncHandler(async (req, res) => {
  ...
  if (!project) {//if we don't have a project, we throw an error
    throw new ApiError(404, "Project not found");
  }
  return res //if project found, send this res
    .status(200)
...
const addMembersToProject = asyncHandler(async (req, res) => {
  ...
  if(!user){//if there's no user
    throw new ApiError(404, "User does not exist")
  }

  await ProjectMember.findByIdAndUpdate(
    { //finding the same user and project, if we find it, we update the role
...
const updateMemberRole = asyncHandler(async (req, res) => {
  ...
  if(!AvailableUserRole.includes(newRole)){//checking if this newRole exists
